...
2.3 Boost: http://sourceforge.net/projects/boost/files/boost/1.61.0/ Download
either the zip or the 7z archive, unpack boost inside your C:\deps folder,
then bootstrap and compile from a Windows command prompt (If `gcc` or other
tools cannot be found, it might work to open `cmd.exe` from within an msys
shell):
